...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, input, context, state):
        ...
        for i, emb_t in enumerate(emb.split(1)):
            ...
            attn_data = attn.data
            #print(attn_data)
            ppl =  ((attn_data*(attn_data.log())).sum(1)).mul(-1)
            #print(ppl)
            idx  = (ppl.le(2.7).unsqueeze(1)).expand_as(attn_output)
            mask = Variable(idx.float())
            #print(mask)
      
            #mask = (((ppl.le(3)).unsqueeze(1)).expand_as(attn_output))
            #mask = ppl.le(3).expand_as(attn_output)
            #print(mask)
            #mask = mask.float()
           # print(mask)
