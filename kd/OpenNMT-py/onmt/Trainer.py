...
class Trainer(object):
    def __init__(self, model, train_iter, valid_iter,
                 ...
                 trunc_size, shard_size, teacher_model):
        """
        Args:
    ...
    def train(self, epoch, report_func=None):
        ...
        for i, batch in enumerate(self.train_iter):
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                outputs_t, attns_t, dec_state_t = \
                    self.teacher_model(src, tgt, src_lengths, dec_state_t)
                #print(src.size())
                #print(tgt.size())
                #print(outputs.size())
                #print(attns.size())
                ...
                batch_stats = self.train_loss.sharded_compute_loss(
                        ...
                        trunc_size, self.shard_size, outputs_t, attns_t)
    ...
    def validate(self):
        ...
        for batch in self.valid_iter:
            ...
            outputs_t, attns_t, _ = self.teacher_model(src, tgt, src_lengths)
        #    print(src) 
            # Compute loss.
            gen_state = onmt.Loss.make_gen_state(
                outputs, batch, attns, (0, batch.tgt.size(0)))
            gen_state_t = onmt.Loss.make_gen_state(
                outputs_t, batch, attns_t, (0, batch.tgt.size(0)))

            _, batch_stats = self.valid_loss(batch, gen_state['output'], gen_state['target'], gen_state_t['output'])
